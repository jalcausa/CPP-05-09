...
* **Exceptions:**
    * Throw `Bureaucrat::GradeTooHighException` or `Bureaucrat::GradeTooLowException` if an invalid grade is provided during instantiation or modification.
* **Member Functions:**
    * `getName()` and `getGrade()` getters.
    * Functions to increment or decrement the grade.
    * *Note:* Incrementing a grade 3 results in grade 2.
* **Operator Overload:**
    * Overload the insertion (`<<`) operator to print: `<name>, bureaucrat grade <grade>.`.
